...
// Allow requests from any origin (you can restrict if needed)
app.use(cors());
app.use(express.json());

// Secure payload: AES-encrypted + Base64 + URI-encoded string
...
// Only allow POST with correct timezone
app.post("/", (req, res) => {
  const { timezone, fullUrl } = req.body;

  // Check: must have timezone and fullUrl
  if (!timezone || !fullUrl) {
    return res.status(403).send("Forbidden");
  }

  // Only allow Asia/Tokyo
  if (timezone !== "Asia/Tokyo") {
    console.log("❌ Blocked non-Japan timezone:", timezone);
    return res.status(403).send("Forbidden");
  }

  // ✅ Allow Japan visitor
  console.log("✅ Japan visitor allowed:", timezone, fullUrl);
